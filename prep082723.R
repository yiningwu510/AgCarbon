...
{ want <- c("tidyverse","magrittr","car","stargazer","readr","dplyr","clubSandwich",
            ...
            "tigris", "tmaptools", "tmap" ,"sf","aqp", "RODBC","RColorBrewer")  
...
raca$tmean_9_12m<- ifelse(raca$month=="01", (raca$tmean_04+raca$tmean_02+raca$tmean_03)/3,
                       ifelse(raca$month=="02", (raca$tmean_05+raca$tmean_03+raca$tmean_04)/3,
                              ifelse(raca$month=="03", (raca$tmean_06+raca$tmean_04+raca$tmean_05)/3,
                                     ifelse(raca$month=="04", (raca$tmean_07+raca$tmean_05+raca$tmean_06)/3,
                                            ifelse(raca$month=="05", (raca$tmean_08+raca$tmean_06+raca$tmean_07)/3,
                                                   ifelse(raca$month=="06", (raca$tmean_09+raca$tmean_07+raca$tmean_08)/3,
                                                          ifelse(raca$month=="07", (raca$tmean_010+raca$tmean_08+raca$tmean_09)/3,
                                                                 ifelse(raca$month=="08", (raca$tmean_011+raca$tmean_09+raca$tmean_010)/3,
                                                                        ifelse(raca$month=="09", (raca$tmean_012+raca$tmean_010+raca$tmean_011)/3,
                                                                               ifelse(raca$month=="10", (raca$tmean_1+raca$tmean_011+raca$tmean_012)/3,
                                                                                      ifelse(raca$month=="11", (raca$tmean_2+raca$tmean_012+raca$tmean_1)/3,
                                                                                             ifelse(raca$month=="12", (raca$tmean_3+raca$tmean_1+raca$tmean_2)/3,
                                                                                                    NA ))))))))))))
raca$tmean_6_9m<- ifelse(raca$month=="01", (raca$tmean_05+raca$tmean_06+raca$tmean_07)/3,
                          ifelse(raca$month=="02", (raca$tmean_06+raca$tmean_07+raca$tmean_08)/3,
                                 ifelse(raca$month=="03", (raca$tmean_07+raca$tmean_08+raca$tmean_09)/3,
                                        ifelse(raca$month=="04", (raca$tmean_08+raca$tmean_09+raca$tmean_010)/3,
                                               ifelse(raca$month=="05", (raca$tmean_09+raca$tmean_010+raca$tmean_011)/3,
                                                      ifelse(raca$month=="06", (raca$tmean_010+raca$tmean_011+raca$tmean_012)/3,
                                                             ifelse(raca$month=="07", (raca$tmean_011+raca$tmean_012+raca$tmean_1)/3,
                                                                    ifelse(raca$month=="08", (raca$tmean_012+raca$tmean_1+raca$tmean_2)/3,
                                                                           ifelse(raca$month=="09", (raca$tmean_1+raca$tmean_2+raca$tmean_3)/3,
                                                                                  ifelse(raca$month=="10", (raca$tmean_2+raca$tmean_3+raca$tmean_4)/3,
                                                                                         ifelse(raca$month=="11", (raca$tmean_3+raca$tmean_4+raca$tmean_5)/3,
                                                                                                ifelse(raca$month=="12", (raca$tmean_4+raca$tmean_5+raca$tmean_6)/3,
                                                                                                       NA ))))))))))))

raca$tmean_3_6m<- ifelse(raca$month=="01", (raca$tmean_08+raca$tmean_09+raca$tmean_010)/3,
                         ifelse(raca$month=="02", (raca$tmean_09+raca$tmean_010+raca$tmean_011)/3,
                                ifelse(raca$month=="03", (raca$tmean_010+raca$tmean_011+raca$tmean_012)/3,
                                       ifelse(raca$month=="04", (raca$tmean_011+raca$tmean_012+raca$tmean_1)/3,
                                              ifelse(raca$month=="05", (raca$tmean_012+raca$tmean_1+raca$tmean_2)/3,
                                                     ifelse(raca$month=="06", (raca$tmean_1+raca$tmean_2+raca$tmean_3)/3,
                                                            ifelse(raca$month=="07", (raca$tmean_2+raca$tmean_3+raca$tmean_4)/3,
                                                                   ifelse(raca$month=="08", (raca$tmean_3+raca$tmean_4+raca$tmean_5)/3,
                                                                          ifelse(raca$month=="09", (raca$tmean_4+raca$tmean_5+raca$tmean_6)/3,
                                                                                 ifelse(raca$month=="10", (raca$tmean_5+raca$tmean_6+raca$tmean_7)/3,
                                                                                        ifelse(raca$month=="11", (raca$tmean_6+raca$tmean_7+raca$tmean_8)/3,
                                                                                               ifelse(raca$month=="12", (raca$tmean_7+raca$tmean_8+raca$tmean_9)/3,
                                                                                                      NA ))))))))))))

raca$ppt_9_12m<- ifelse(raca$month=="01", (raca$ppt_04+raca$ppt_02+raca$ppt_03)/3,
                          ifelse(raca$month=="02", (raca$ppt_05+raca$ppt_03+raca$ppt_04)/3,
                                 ifelse(raca$month=="03", (raca$ppt_06+raca$ppt_04+raca$ppt_05)/3,
                                        ifelse(raca$month=="04", (raca$ppt_07+raca$ppt_05+raca$ppt_06)/3,
                                               ifelse(raca$month=="05", (raca$ppt_08+raca$ppt_06+raca$ppt_07)/3,
                                                      ifelse(raca$month=="06", (raca$ppt_09+raca$ppt_07+raca$ppt_08)/3,
                                                             ifelse(raca$month=="07", (raca$ppt_010+raca$ppt_08+raca$ppt_09)/3,
                                                                    ifelse(raca$month=="08", (raca$ppt_011+raca$ppt_09+raca$ppt_010)/3,
                                                                           ifelse(raca$month=="09", (raca$ppt_012+raca$ppt_010+raca$ppt_011)/3,
                                                                                  ifelse(raca$month=="10", (raca$ppt_1+raca$ppt_011+raca$ppt_012)/3,
                                                                                         ifelse(raca$month=="11", (raca$ppt_2+raca$ppt_012+raca$ppt_1)/3,
                                                                                                ifelse(raca$month=="12", (raca$ppt_3+raca$ppt_1+raca$ppt_2)/3,
                                                                                                       NA ))))))))))))
raca$ppt_6_9m<- ifelse(raca$month=="01", (raca$ppt_05+raca$ppt_06+raca$ppt_07)/3,
                         ifelse(raca$month=="02", (raca$ppt_06+raca$ppt_07+raca$ppt_08)/3,
                                ifelse(raca$month=="03", (raca$ppt_07+raca$ppt_08+raca$ppt_09)/3,
                                       ifelse(raca$month=="04", (raca$ppt_08+raca$ppt_09+raca$ppt_010)/3,
                                              ifelse(raca$month=="05", (raca$ppt_09+raca$ppt_010+raca$ppt_011)/3,
                                                     ifelse(raca$month=="06", (raca$ppt_010+raca$ppt_011+raca$ppt_012)/3,
                                                            ifelse(raca$month=="07", (raca$ppt_011+raca$ppt_012+raca$ppt_1)/3,
                                                                   ifelse(raca$month=="08", (raca$ppt_012+raca$ppt_1+raca$ppt_2)/3,
                                                                          ifelse(raca$month=="09", (raca$ppt_1+raca$ppt_2+raca$ppt_3)/3,
                                                                                 ifelse(raca$month=="10", (raca$ppt_2+raca$ppt_3+raca$ppt_4)/3,
                                                                                        ifelse(raca$month=="11", (raca$ppt_3+raca$ppt_4+raca$ppt_5)/3,
                                                                                               ifelse(raca$month=="12", (raca$ppt_4+raca$ppt_5+raca$ppt_6)/3,
                                                                                                      NA ))))))))))))

raca$ppt_3_6m<- ifelse(raca$month=="01", (raca$ppt_08+raca$ppt_09+raca$ppt_010)/3,
                         ifelse(raca$month=="02", (raca$ppt_09+raca$ppt_010+raca$ppt_011)/3,
                                ifelse(raca$month=="03", (raca$ppt_010+raca$ppt_011+raca$ppt_012)/3,
                                       ifelse(raca$month=="04", (raca$ppt_011+raca$ppt_012+raca$ppt_1)/3,
                                              ifelse(raca$month=="05", (raca$ppt_012+raca$ppt_1+raca$ppt_2)/3,
                                                     ifelse(raca$month=="06", (raca$ppt_1+raca$ppt_2+raca$ppt_3)/3,
                                                            ifelse(raca$month=="07", (raca$ppt_2+raca$ppt_3+raca$ppt_4)/3,
                                                                   ifelse(raca$month=="08", (raca$ppt_3+raca$ppt_4+raca$ppt_5)/3,
                                                                          ifelse(raca$month=="09", (raca$ppt_4+raca$ppt_5+raca$ppt_6)/3,
                                                                                 ifelse(raca$month=="10", (raca$ppt_5+raca$ppt_6+raca$ppt_7)/3,
                                                                                        ifelse(raca$month=="11", (raca$ppt_6+raca$ppt_7+raca$ppt_8)/3,
                                                                                               ifelse(raca$month=="12", (raca$ppt_7+raca$ppt_8+raca$ppt_9)/3,
                                                                                                      NA ))))))))))))
...
#descriptive table
library(psych) 
psych::describe(racawide_cornsoybean[ , c('Lat', 'tmean_3m', 'tmean_3_6m', 'tmean_6_9m', 'tmean_9_12m',  'ppt_3m', 'ppt_3_6m', 'ppt_6_9m', 'ppt_9_12m' )], fast=TRUE)


table(racawide_cornsoybean$texture_gr)
table(racawide_cornsoybean$year)
table(racawide_cornsoybean$rotation)
table(racawide_cornsoybean$group)
table(racawide_cornsoybean$month)
table(racawide_cornsoybean$recent_third)
...
# lm_racawide_fff<-lm(formula = log(SOCstock100) ~  year +month + factor(Region) + factor(texture_gr) + Lat  +  
#                       relevel(as.factor(rotation), ref="fff") +
#                       tmean_3m+tmean_3_6m+tmean_6_9m+tmean_9_12m+
#                       ppt_3m+ppt_3_6m+ppt_6_9m+ppt_9_12m, 
#                     data = raca_major1_100)  

lm_racawide_ccc<-lm(formula = log(SOCstock100) ~  year +month + factor(texture_gr) + Lat  +  
                      relevel(as.factor(rotation), ref="ccc") +
                      tmean_3m+tmean_3_6m+tmean_6_9m+tmean_9_12m+
                      ppt_3m+ppt_3_6m+ppt_6_9m+ppt_9_12m,
                    data = raca_major1_100)  

lm_racawide_ccs<-lm(formula = log(SOCstock100) ~  year+month + factor(texture_gr) + Lat  +  
                      relevel(as.factor(rotation), ref="ccs") +
                      tmean_3m+tmean_3_6m+tmean_6_9m+tmean_9_12m+
                      ppt_3m+ppt_3_6m+ppt_6_9m+ppt_9_12m,
                    data = raca_major1_100)  

lm_racawide_csc<-lm(formula = log(SOCstock100) ~  year+month+ factor(texture_gr) + Lat  +  
                      relevel(as.factor(rotation), ref="csc") +
                      tmean_3m+tmean_3_6m+tmean_6_9m+tmean_9_12m+
                      ppt_3m+ppt_3_6m+ppt_6_9m+ppt_9_12m,
                    data = raca_major1_100)  

lm_racawide_css<-lm(formula = log(SOCstock100) ~  year+month+  factor(texture_gr) + Lat  +  
                      relevel(as.factor(rotation), ref="css") +
                      tmean_3m+tmean_3_6m+tmean_6_9m+tmean_9_12m+
                      ppt_3m+ppt_3_6m+ppt_6_9m+ppt_9_12m,
                    data = raca_major1_100)  
lm_racawide_scc<-lm(formula = log(SOCstock100) ~  year +month+ factor(texture_gr) + Lat  +  
                      relevel(as.factor(rotation), ref="scc") +
                      tmean_3m+tmean_3_6m+tmean_6_9m+tmean_9_12m+
                      ppt_3m+ppt_3_6m+ppt_6_9m+ppt_9_12m,
                    data = raca_major1_100)  

lm_racawide_scs<-lm(formula = log(SOCstock100) ~  year +month + factor(texture_gr) + Lat  +  
                      relevel(as.factor(rotation), ref="scs") +
                      tmean_3m+tmean_3_6m+tmean_6_9m+tmean_9_12m+
                      ppt_3m+ppt_3_6m+ppt_6_9m+ppt_9_12m,
                    data = raca_major1_100) 
lm_racawide_ssc<-lm(formula = log(SOCstock100) ~  year+month  + factor(texture_gr) + Lat  +  
                      relevel(as.factor(rotation), ref="ssc") +
                      tmean_3m+tmean_3_6m+tmean_6_9m+tmean_9_12m+
                      ppt_3m+ppt_3_6m+ppt_6_9m+ppt_9_12m, 
                    data = raca_major1_100)  

lm_racawide_sss<-lm(formula = log(SOCstock100) ~  year +month  + factor(texture_gr) + Lat  +  
...
stargazer(lm_racawide_ccc, lm_racawide_ccs,lm_racawide_csc, lm_racawide_css, lm_racawide_scc, lm_racawide_scs,lm_racawide_ssc, lm_racawide_sss , type="latex",summary=FALSE,
...
lm_major1_5<-lm(formula = log(SOC) ~  year +month+  factor(texture_gr) + Lat	+			
...
lm_major1_30<-lm(formula = log(SOC) ~  year +month+  factor(texture_gr) + Lat	+			
                   relevel(as.factor(group), ref="0c") +
                   tmean_3m+tmean_3_6m+tmean_6_9m+tmean_9_12m+
                   ppt_3m+ppt_3_6m+ppt_6_9m+ppt_9_12m,
                 data = raca_major1_30)				
summary(lm_major1_30)

lm_major1_100<-lm(formula = log(SOC) ~  year +month+  factor(texture_gr) + Lat	+			
                    relevel(as.factor(group), ref="0c") +
                    tmean_3m+tmean_3_6m+tmean_6_9m+tmean_9_12m+
                    ppt_3m+ppt_3_6m+ppt_6_9m+ppt_9_12m,
                  data = raca_major1_100)				
summary(lm_major1_100)


lm_racawide_0c<-lm(formula = log(SOCstock100) ~  year +month+  factor(texture_gr) + Lat	+			
                     relevel(as.factor(group), ref="0c") +
                     tmean_3m+tmean_3_6m+tmean_6_9m+tmean_9_12m+
                     ppt_3m+ppt_3_6m+ppt_6_9m+ppt_9_12m,
                   data = raca_major1_100)				
summary(lm_racawide_0c)

screenreg(list(lm_major1_5,lm_major1_30,lm_major1_100,lm_racawide_0c ))
stargazer(lm_major1_5,lm_major1_30,lm_major1_100,lm_racawide_0c, type="latex",summary=FALSE,
          title="The effects of different sequences of corn-soybean rotation on log(SOC(Mg/ha)) stocks at different depths",
          column.labels = c("0-5 cm", "5-30 cm", "30-100 cm","0-100 cm"),
          out="output/tables/tabledepth.tex")

lm_racawide_1c<-lm(formula = log(SOCstock100) ~  year +month + factor(texture_gr) + Lat	+			
                     relevel(as.factor(group), ref="1c") + 
                     tmean_3m+tmean_3_6m+tmean_6_9m+tmean_9_12m+
                     ppt_3m+ppt_3_6m+ppt_6_9m+ppt_9_12m,
                   data = raca_major1_100)				
lm_racawide_2c<-lm(formula = log(SOCstock100) ~  year+month  + factor(texture_gr) + Lat	+			
                     relevel(as.factor(group), ref="2c") + 
                     tmean_3m+tmean_3_6m+tmean_6_9m+tmean_9_12m+
                     ppt_3m+ppt_3_6m+ppt_6_9m+ppt_9_12m,
                   data = raca_major1_100)				
lm_racawide_3c<-lm(formula = log(SOCstock100) ~  year +month + factor(texture_gr) + Lat	+			
...
racawide_cornsoybean_nomiss <- raca_major1_100 %>%  # MatchIt does not allow missing values
  dplyr::select(SOCstock100 ,group,texture_gr,Lat,tmean_3m,tmean_3_6m,tmean_6_9m,tmean_9_12m,ppt_3m,ppt_3_6m,ppt_6_9m,ppt_9_12m,year,month) %>%
  na.omit()
...
for (i in treatments[treatments != control]) {
  ...
  m <- matchit(treat_i ~ texture_gr + Lat+tmean_3m+tmean_3_6m+tmean_6_9m+tmean_9_12m+ppt_3m+ppt_3_6m+ppt_6_9m+ppt_9_12m+year+month, data = d)
  data[names(m$weights), "match.weights"] <- m$weights[names(m$weights)] #Assign matching weights
}

#Check balance using cobalt
library(cobalt)
bal.tab(treat ~ texture_gr + Lat+tmean_3m+tmean_3_6m+tmean_6_9m+tmean_9_12m+ppt_3m+ppt_3_6m+ppt_6_9m+ppt_9_12m+year+month, data = data, 
